...
## Assessment integration deliverables

The final integration is implemented without replacing the existing algorithms:

- `src/q_rescue/services/application_service.py` — shared orchestration boundary.
- `src/q_rescue/api/contracts.py` — versioned AI/data contracts and validation.
- `src/q_rescue/api/prediction_api.py` — AI Prediction Layer API adapter.
- `docs/integration_boundaries.md` — module boundaries and contract definitions.
- `docs/ethical_social_impact.md` — emergency-response risk, bias, automation
  limits, explainability and human oversight.
- `tests/unit/test_api_contracts.py` and
  `tests/unit/test_application_service.py` — integration/contract coverage.

Run `pytest` to validate the complete package.
